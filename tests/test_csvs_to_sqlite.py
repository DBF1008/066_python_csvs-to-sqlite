...
def test_multiple_files_with_shape_and_fixed_column():
    """
    Regression test: batch-importing multiple CSVs with --shape and
    --filename-column / --fixed-column must not let the shape string
    or sql_type_overrides leak between files.
    """
    runner = CliRunner()
    with runner.isolated_filesystem():
        open("election1.csv", "w").write(CSV)
        open("election2.csv", "w").write(CSV)
        result = runner.invoke(
            cli.cli,
            [
                ".",
        ...
        conn = sqlite3.connect("multi-shape.db")
        ...
        for tbl in tables:
            cols = conn.execute(
                'PRAGMA table_info("{}")'.format(tbl)
            ).fetchall()
            col_names = [c[1] for c in cols]
            col_types = {c[1]: c[2] for c in cols}

            # Correct columns present (no leaked columns)
            assert col_names == ["Cty", "Vts", "source", "batch"], (
                "Table {} has columns {}, expected ['Cty', 'Vts', 'source', 'batch']".format(
                    tbl, col_names
                )
            )
            # Vts type override (REAL) applied to every table
            assert col_types["Vts"] == "REAL", (
                "Table {} has Vts type {} instead of REAL".format(tbl, col_types["Vts"])
            )
            ...
            assert count == 6, "Table {} has {} rows, expected 6".format(tbl, count)

            # Fixed column value correct
            batch_val = conn.execute(
                'select distinct batch from "{}"'.format(tbl)
            ).fetchone()[0]
            assert batch_val == "test-batch"

        # Source column differentiates the two files
        sources = set()
        for tbl in tables:
            row = conn.execute(
                'select distinct source from "{}"'.format(tbl)
            ).fetchone()
            sources.add(row[0])
        assert len(sources) == 2, "Expected 2 distinct source values, got {}".format(
            sources
        )
