...
def cli(
    ...
    for name, path in csvs.items():
        try:
            file_shape = shape
            df = load_csv(
                path, separator, skip_errors, quoting, file_shape, just_strings=just_strings
            )
            ...
            if fixed_columns_float:
                for colname, value in fixed_columns_float:
                    ...
                    if file_shape:
                        file_shape += ",{}".format(colname)
            df.attrs["_sql_type_overrides"] = apply_shape(df, file_shape)
    ...
    for df in refactored:
        ...
        if table_exists(conn, df.table_name):
            ...
            to_sql_with_foreign_keys(
                ...
                df.table_name,
                foreign_keys,
                df.attrs.get("_sql_type_overrides"),
